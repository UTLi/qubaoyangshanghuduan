...
//        
//        _arrowButton = [[UIImageView alloc] initWithFrame:CGRectMake(functionButtonX, DOT_COORDINATE, rect.size.width/2, ARROW_BUTTON_WIDTH)];
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake((rect.size.width/num-width)/2, NAV_TAB_BAR_HEIGHT - 3.0f,width , 3.0f)];
    
    
#warning line color
    _line.backgroundColor = MainPurpleColor;
...
#warning 设置初始button颜色
        if (index==0) {
            [button setTitleColor:MainPurpleColor forState:UIControlStateNormal];
        }else{
            [button setTitleColor:[UIColor colorWithRed:64/255.0 green:64/255.0 blue:64/255.0 alpha:1] forState:UIControlStateNormal];
        }
        
    
        button.frame = CGRectMake(buttonX, DOT_COORDINATE,rect.size.width/num , NAV_TAB_BAR_HEIGHT);
...
//        [button setTintColor:[UIColor redColor] forState:UIControlStateNormal];
        button.titleLabel.font=[UIFont systemFontOfSize:16];
        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_navgationTabBar addSubview:button];
        
        [_items addObject:button];
       // buttonX += [widths[index] floatValue]+40;
        
        buttonX+=rect.size.width/num;
    }
...
//        _line.frame = CGRectMake(button.frame.origin.x + 2.0f+, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue] - 4.0f, _line.frame.size.height);
    ...
    [UIView animateWithDuration:0.2f animations:^{
//        _line.frame = CGRectMake(button.frame.origin.x + 2.0f+button.frame.origin.x + 2.0f+(rect.size.width/2 - [_itemsWidth[currentItemIndex] floatValue] - 4.0f)/2, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue] - 4.0f, _line.frame.size.height);
         _line.frame = CGRectMake((button.frame.origin.x + 2.0f+button.frame.origin.x + 2.0f+(rect.size.width/num - [_itemsWidth[currentItemIndex] floatValue] ))/2, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue] - 4.0f, _line.frame.size.height);
        

      // button.frame.origin.x + 2.0f+(rect.size.width/2 - [_itemsWidth[currentItemIndex] floatValue] - 4.0f)/2
    }];
}
